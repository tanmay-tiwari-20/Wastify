...
export default function RewardsPage() {
    ...
    const fetchUserDataAndRewards = async () => {
      ...
      try {
        if (user) {
          ...
          const fetchedRewards = await getAvailableRewards(user.id);
  ...
  }, [isSignedIn, user, router]); // Add router to dependencies to re-run effect on user not logged in

  const handleRedeemReward = async (rewardId: number) => {
    const { user, isSignedIn } = useUser(); // Get the user data from Clerk
    if (!isSignedIn) {
  ...
  const refreshUserData = async () => {
    const { user, isSignedIn } = useUser(); // Get the user data from Clerk
    if (!isSignedIn || !user) {
      return; // If the user is not signed in, don't fetch user data
    }
